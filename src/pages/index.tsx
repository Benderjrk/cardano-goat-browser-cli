  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (command !== undefined) {
      if (data.command === 'clear' && command[0] !== undefined) {
        ...
        if (command.length > 2) {
          setCommand([...command.slice(0,1),...command.slice(-2), data.command]);
        } else {
      ...
      <main className="gap-1 min-h-screen p-4 bg-zinc-900 cursor-text divide-dashed hover:divide-y">
        ...
        {command &&
          command.map(function (d, idx) {
            ...
            if (d === 'man')
              return (
                <><div key={idx} className="text-zinc-400 mb-2">
                  <span className="text-green-500">$ </span>Command: {d}
                </div><div key={idx} className="text-zinc-400 text-center items-center">
                    ...
                    <div>
                      ...
                      <div className="mb-2">social links - print out links to social media</div>
                    </div>
                  </div></>
              );
            if (d === 'buy goat')
            return (
              <div key={idx} className="text-zinc-400">
                ...
                <div className="text-zinc-400 mb-2">
                <span className="text-green-500">$ </span><span className="text-cyan-400">To purchase a Goat. Send 25 $ADA exactly to:</span>
                </div>
                <div className="text-zinc-400 text-center items-center mb-2 break-words">
                <span className="text-indigo-400 select-all">addr1qx8j0xzjnvputf6dnfdyghmt4tguh622pmhwu69zdd8djfxkd2pzw443r70hsmrpdh4tnqelhdvssq5zm75fw8ht6psqgn4xep</span>
                </div>
                <div className="text-zinc-400 text-center items-center mb-2">
                <span className="text-red-400">Do not send funds from exchanges or funds can be lost. Private wallets only.</span>
                </div>
              </div>
            );
            return (
